...
(function() {
	...
	as = angular.module('exampleApp', [ 'ngRoute', 'ngResource', 'ngCookies',
			'exampleApp.i18n', 'exampleApp.services', 'exampleApp.controllers',
			'exampleApp.directives', 'exampleApp.filters' ]);
	...
	as.config(function($routeProvider, $httpProvider) {
		...
		// configure $http to show a login dialog whenever a 401 unauthorized
		// response arrives
	...
	as.run(function($rootScope, $http, $route, $location, base64) {
		// make current message accessible to root scope and therefore all
		// scopes
		...
		/**
		 * On 'logoutRequest' invoke logout on the server and broadcast
		 * 'event:loginRequired'.
		 */
		...
		angular.forEach($route.routes, function(route, path) {
			// push route onto routesOpenToPublic if it has a truthy
			// publicAccess value
